...
export default function ShippingSummary({ subtotalAmount, compact = false }: Props) {
    ...
    >
      ...
      <dl className="mt-4 grid gap-2 text-sm">
        <div className="flex items-center justify-between gap-4">
          <dt className="font-bold text-ink-600">Standard shipping</dt>

          <dd className="text-right font-black text-ink-900">
            {shippingState.qualifiesForFreeShipping ? 'Free' : 'Weight-based estimate'}
          </dd>
        </div>

        <div className="flex items-center justify-between gap-4 border-t border-brand-200 pt-2">
          <dt className="font-bold text-ink-600">Final shipping charge</dt>

          <dd className="text-right font-black text-ink-900">
            {shippingState.qualifiesForFreeShipping ? 'Included' : 'Calculated at checkout'}
          </dd>
        </div>
      </dl>

      <p className="mt-3 text-xs font-bold leading-5 text-ink-600">
        Standard shipping is estimated from the order's packed weight and U.S. destination. The
        amount is shown before payment.
      </p>

      <a
        href={commerceConfig.shipping.policyHref}

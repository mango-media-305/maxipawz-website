...
function getStatusClass(reservation: AdminInventoryReservation): string {
  if (reservation.expirationPastDue) {
...
function StatusBadge({ reservation }: { reservation: AdminInventoryReservation }) {
  return (
    <span
      className={[
    ...
    >
      {reservation.expirationPastDue ? 'Past Expiry' : formatStatusLabel(reservation.status)}
    </span>
  );
}
...
function Metric({
  ...
  return (
    <div className="rounded-2xl border border-sand bg-cream-soft p-3">
      <dt className="text-xs font-extrabold tracking-[0.06em] text-ink-500 uppercase">{label}</dt>

      <dd
        className={['mt-1 text-xl font-black', emphasis ? 'text-brand-700' : 'text-ink-900'].join(
          ' ',
        )}
      >
        {value}
...
function matchesFilter(reservation: AdminInventoryReservation, filter: ReservationFilter): boolean {
  switch (filter) {
    case 'all':
      return true;
    ...
    case 'past-due':
      return reservation.expirationPastDue;
...
function matchesSearch(reservation: AdminInventoryReservation, query: string): boolean {
  ...
  const itemValues = reservation.items.flatMap((item) => [
    item.productName,
    item.productSlug,
    item.variantLabel,
    item.variantId,
...
function ReservationCard({ reservation }: { reservation: AdminInventoryReservation }) {
  const terminalTimestamp =
    reservation.completedAt ?? reservation.releasedAt ?? reservation.expiredAt;
    ...
    <article className="rounded-4xl border border-sand bg-white-warm p-5 shadow-card sm:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="lg:text-right">
          <p className="text-xs font-extrabold tracking-[0.07em] text-ink-500 uppercase">Created</p>

          <p className="mt-1 text-sm font-bold text-ink-700">{formatDate(reservation.createdAt)}</p>
        </div>
      </div>

      {reservation.expirationPastDue && (
        <div className="mt-5 rounded-2xl border border-danger-100 bg-danger-50 p-4">
          ...
          <p className="mt-1 text-sm leading-6 text-ink-700">
            The reservation remains read-only here. The Stripe lifecycle/reconciliation system is
      ...
      {reservation.status === 'payment-pending' && (
        <div className="mt-5 rounded-2xl border border-accent-200 bg-accent-50 p-4">
          <p className="font-extrabold text-accent-700">Payment still pending</p>

          <p className="mt-1 text-sm leading-6 text-ink-700">
            Inventory remains reserved while the payment method is unresolved. This dashboard
            intentionally cannot release it manually.
          </p>
        </div>
      )}
      ...
      <dl className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-sand bg-cream-soft p-3">
          ...
          <dd className="mt-1 break-all font-mono text-xs font-bold text-ink-900">
            {reservation.stripeSessionId ?? 'Not attached'}
          </dd>
        </div>
      ...
      <section className="mt-5">
        <h3 className="text-sm font-extrabold text-ink-900">Reserved Items</h3>
      ...
      {reservation.releaseReason && (
        <div className="mt-5 rounded-2xl border border-sand bg-cream-soft p-4 text-sm leading-6 text-ink-700">
          <strong>Release reason:</strong> {formatStatusLabel(reservation.releaseReason)}
        </div>
      )}

      {terminalTimestamp && (
        <p className="mt-5 text-xs font-bold text-ink-500">
          Lifecycle completed: {formatDate(terminalTimestamp)}
        </p>
      )}
    </article>
  );
...
export default function AdminReservations() {
  ...
  const [data, setData] = useState<AdminInventoryReservationsData | null>(null);
  ...
  const [filter, setFilter] = useState<ReservationFilter>('holding');

  async function loadReservations(adminToken: string, quiet = false) {
    if (quiet) {
      setRefreshing(true);
    } else {
      setLoading(true);
    ...
    try {
      ...
      const payload = (await response
        .json()
        .catch(() => null)) as AdminInventoryReservationsResponse | null;
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      ...
      void loadReservations(savedToken).catch(() => undefined);
    }
  }, []);
  ...
  const visibleReservations = useMemo(
    () =>
      (data?.reservations ?? []).filter(
        (reservation) => matchesFilter(reservation, filter) && matchesSearch(reservation, query),
      ),
    [data, filter, query],
  );
  ...
  if (!token) {
    return (
      <section className="mx-auto max-w-xl rounded-[2.5rem] border border-brand-200 bg-white-warm p-6 shadow-card sm:p-8">
        ...
        <h1 className="mt-3 text-3xl text-ink-900">Inventory Reservations</h1>

        <p className="mt-3 text-sm leading-6 text-ink-600">
          Enter the private administrator token to inspect Checkout inventory holds and reservation
          history.
        </p>
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            void loadReservations(tokenInput).catch(() => undefined);
          }}
        >
          ...
          >
            {loading ? 'Loading…' : 'Open Reservations'}
          </button>
        </form>
  ...
  if (!data) {
    return (
      <section className="rounded-4xl border border-sand bg-white-warm p-6 shadow-card">
        <p className="font-bold text-ink-700">Reservation data is unavailable.</p>

        {error && <p className="mt-3 text-sm font-bold text-danger-700">{error}</p>}

        <button
          type="button"
          className="mt-4 rounded-full bg-brand-500 px-5 py-3 font-extrabold text-white"
          onClick={() => {
            void loadReservations(token).catch(() => undefined);
          }}
        >
          Try Again
        </button>
  ...
  const filters: {
  ...
  }[] = [
    {
      ...
      count: data.summary.active + data.summary.paymentPending,
    },
...

    {
      ...
      count: data.summary.paymentPending,
    },
...

    {
      ...
      count: data.summary.expirationPastDue,
    },
  ...
  return (
    <div className="grid gap-8">
      <header className="rounded-4xl border border-sand bg-white-warm p-5 shadow-card sm:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl text-ink-900 sm:text-4xl">Inventory Reservations</h1>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={() => {
                void loadReservations(token, true).catch(() => undefined);
              }}
            >
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric
          label="Holding Stock"
          value={data.summary.active + data.summary.paymentPending}
          emphasis
        />

        <Metric label="Held Units" value={data.summary.heldUnits} emphasis />

        <Metric label="Past Expiry" value={data.summary.expirationPastDue} />
      </section>

      {data.summary.expirationPastDue > 0 && (
        <section className="rounded-4xl border border-danger-100 bg-danger-50 p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-ink-700">
            At least one active reservation is beyond its Checkout expiration timestamp. The
            automated reconciler should normally clear these safely. This page does not bypass that
            lifecycle.
          </p>
        </section>
      )}
        ...
        <div className="rounded-4xl border border-sand bg-white-warm p-5 shadow-card sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl text-ink-900">Checkout Holds</h2>
            </div>

            <label className="grid gap-1.5 lg:w-80">
              <span className="form-label">Search reservations</span>
          ...
          {data.truncated && (
            <p className="mt-4 text-xs font-bold text-ink-500">
              Showing the latest {data.displayLimit} reservations of {data.summary.total} total.
            </p>
          )}
        </div>
        ...
        <div className="mt-5 grid gap-5">
          {visibleReservations.length > 0 ? (
            ...
            <div className="rounded-4xl border border-sand bg-white-warm p-8 text-center shadow-card">
              <p className="font-extrabold text-ink-900">No reservations match this view.</p>

              <p className="mt-2 text-sm text-ink-600">Try another filter or search term.</p>
            </div>
          )}
        </div>
      </section>

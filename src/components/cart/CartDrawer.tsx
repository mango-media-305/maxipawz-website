...
export default function CartDrawer() {
  ...
  const totals = useMemo(() => getCartTotals(resolvedLines), [resolvedLines]);
        ...
        <header className="flex shrink-0 items-center justify-between gap-4 border-b border-sand bg-white-warm px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 id="cart-drawer-title" className="mt-1 text-xl text-ink-900">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-4 sm:p-5">
          {!hydrated ? (
            <div className="rounded-card-lg border border-sand bg-white-warm p-6 text-center shadow-sm">
              <p className="font-bold text-ink-600">Loading your cart…</p>
            </div>
            ...
            <div className="grid gap-3">
              {resolvedLines.map((item) => (
                <CartItem key={item.key} item={item} compact={true} />
              ))}
            </div>
          )}
        </div>
        ...
        {resolvedLines.length > 0 && (
          <footer className="shrink-0 overflow-y-auto border-t border-sand bg-white-warm p-5">
            ...
            <div className="flex items-center justify-between gap-4">
              <span className="font-bold text-ink-600">Merchandise subtotal</span>

              <span className="text-2xl font-black text-ink-900">
                {formatCartAmount(totals.subtotalAmount)}
              </span>
            </div>
            ...
            <div className="mt-5">
              <CheckoutButton lines={resolvedLines} compact={true} />
            </div>

...
import { removeCartLine, setCartLineQuantity } from '../../stores/cart';
...
export default function CartItem({ item, compact = false }: Props) {
  ...
  const effectiveAvailability = product
    ? getEffectiveProductAvailability(product, variant)
    : undefined;

  const inventoryTrackingEnabled = Boolean(
    product &&
      effectiveAvailability === 'in-stock' &&
      isInventoryTrackingEnabledForSelection(product, variant),
  );
  ...
  const inventoryLookup = useProductInventory({
    ...
    enabled: inventoryTrackingEnabled,
  });
  ...
  const canIncrease =
    ...
    (!inventoryTrackingEnabled ||
      (inventoryLookup.status === 'ready' &&
        inventoryLookup.inventory?.tracked === true &&
        liveAvailable !== null &&
        line.quantity < liveAvailable));

  let displayedIssue = issue;
  ...
  let stockMessageClass = 'text-ink-500';
  ...
  function changeQuantity(nextQuantity: number): void {
    setCartLineQuantity(line.productSlug, line.variantId, nextQuantity);
  }

  function removeItem(): void {
    removeCartLine(line.productSlug, line.variantId);
  }
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              {productHref ? (
                ...
                <h3 className={`font-black text-ink-900 ${compact ? 'text-sm' : 'text-lg'}`}>
                  {productName}
                </h3>
              )}

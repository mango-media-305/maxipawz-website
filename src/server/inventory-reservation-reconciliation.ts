...
const ORPHAN_RESERVATION_GRACE_MS =
    10 *
    60 *
    1000;

const PAYMENT_PENDING_RECHECK_MS =
    10 *
    60 *
    1000;
...
export interface InventoryReservationReconciliationSummary {
    ...
    details:
        InventoryReservationReconciliationDetail[];
}
...
function normalizeDate(
    ...
    if (
        ...
        throw new Error(
            `Inventory reservation field "${fieldName}" contains an invalid timestamp.`,
        );
    }
...
function normalizeInteger(
    ...
    if (
        ...
        throw new Error(
            `Inventory reservation field "${fieldName}" contains an invalid integer.`,
        );
    }
...
> {
    ...
    const orphanCutoff =
        new Date(
            now.getTime() -
            ORPHAN_RESERVATION_GRACE_MS,
        );

    const paymentPendingCutoff =
        new Date(
            now.getTime() -
            PAYMENT_PENDING_RECHECK_MS,
        );
    ...
    return rows.map(
        (row) => {
            const candidate =
                row as
                    ReconciliationCandidateDatabaseRow;
    ...
    try {
        ...
        for (
            ...
            if (
                ...
                throw new Error(
                    `Inventory reservation ${reservationId} contains an invalid quantity.`,
                );
            }
            ...
            if (!inventory) {
                throw new Error(
                    `Reserved inventory for reservation ${reservationId} could not be found.`,
                );
            }
...
> {
    ...
    validateStripeSessionOwnership(
        session,
        candidate.id,
    );
...
> {
    if (
        ...
        return expireOrphanedInventoryReservation(
            candidate.id,
            now,
        );
    }

    return reconcileStripeBackedReservation(
        stripe,
        candidate,
        now,
    );
...
function createEmptySummary():
    InventoryReservationReconciliationSummary {
    return {
        scanned:
            0,
...
> {
    const candidates =
        await getReconciliationCandidates(
            now,
        );
    ...
    for (
        ...
        try {
            ...
            console.error(
                'Inventory reservation reconciliation failed for one reservation.',
                {
                    reservationId:
                        candidate.id,

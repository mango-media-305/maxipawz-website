...
import type { NewsletterLeadRecord, NewsletterResendSyncStatus } from '../../types/newsletter';

import { buildEmailSiteUrl } from './branding';
...
type NewsletterDataMode = 'test' | 'live';

interface NewsletterUnsubscribeRuntimeConfig {
  dataMode: NewsletterDataMode;
...
export class NewsletterUnsubscribeError extends Error {
  readonly code: NewsletterUnsubscribeErrorCode;

  readonly status: number;

  constructor(
    code: NewsletterUnsubscribeErrorCode,
    ...
    this.name = 'NewsletterUnsubscribeError';
...
function getRequiredEnvironmentVariable(name: string): string {
  ...
  if (!value) {
    throw new NewsletterUnsubscribeError('configuration-error', 503, `${name} is missing.`);
  }

  return value;
}

function getNewsletterDataMode(): NewsletterDataMode {
...
function getRuntimeConfig(): NewsletterUnsubscribeRuntimeConfig {
  const contactsApiKey = getRequiredEnvironmentVariable('RESEND_CONTACTS_API_KEY');

  if (!contactsApiKey.startsWith('re_')) {
    throw new NewsletterUnsubscribeError(
      'configuration-error',
      503,
  ...
  const topicId = getRequiredEnvironmentVariable('RESEND_NEWSLETTER_TOPIC_ID');

  const unsubscribeSecret = getRequiredEnvironmentVariable('NEWSLETTER_UNSUBSCRIBE_SECRET');
  ...
  return {
    dataMode: getNewsletterDataMode(),
...
function getLeadStore(dataMode: NewsletterDataMode) {
  return getStore(`maxipawz-newsletter-leads-${dataMode}`, {
    consistency: 'strong',
  });
}

function getLeadKey(emailHash: string): string {
...
function isResendNotFoundError(error: unknown): boolean {
  if (typeof error !== 'object' || error === null) {
    return false;
  }

  const candidate = error as ResendApiErrorLike;

  return candidate.statusCode === 404 || candidate.name === 'not_found';
}
...
function buildTokenPayload(
  ...
  return [TOKEN_VERSION, dataMode, emailHash].join('.');
}
...
function parseToken(
  ...
  config: NewsletterUnsubscribeRuntimeConfig,
): ParsedNewsletterUnsubscribeToken {
  const parts = token.trim().split('.');

  if (parts.length !== 4) {
    throw new NewsletterUnsubscribeError('invalid-token', 400, 'The unsubscribe link is invalid.');
  }

  const [version, mode, emailHash, signature] = parts;

  if (
    version !== TOKEN_VERSION ||
    (mode !== 'test' && mode !== 'live') ||
    !emailHash ||
    !EMAIL_HASH_PATTERN.test(emailHash) ||
    !signature ||
  ...
  const payload = buildTokenPayload(mode, emailHash);

  const expectedSignature = signTokenPayload(payload, config.unsubscribeSecret);
...
async function getLead(
  ...
  const store = getLeadStore(dataMode);

  return (await store.get(getLeadKey(emailHash), {
    type: 'json',
...
async function saveLead(
  ...
  record: NewsletterLeadRecord,
): Promise<void> {
  const store = getLeadStore(dataMode);

  await store.setJSON(getLeadKey(record.emailHash), record);
}
...
async function createResendOptOutContact(
  ...
  if (!result.data?.id) {
    throw new Error('Resend created the opt-out Contact but did not return a Contact ID.');
  }
...
async function updateResendOptOutContact(
  ...
  const contactResult = await resend.contacts.update({
    id: contactId,
  ...
  return contactResult.data?.id ?? contactId;
}
...
async function syncOptOutToResend(
  ...
  const resend = new Resend(config.contactsApiKey);
   ...
   */
  const lookupResult = await resend.contacts.get({
    email: lead.email,
  });

  if (lookupResult.error) {
    if (isResendNotFoundError(lookupResult.error)) {
      return await createResendOptOutContact(resend, config, lead);
    }
  ...
  if (!lookupResult.data?.id) {
    throw new Error('Resend returned an opt-out Contact without a Contact ID.');
  }

  return await updateResendOptOutContact(resend, config, lookupResult.data.id);
}

async function getLeadFromToken(token: string): Promise<{
  config: NewsletterUnsubscribeRuntimeConfig;

  lead: NewsletterLeadRecord;

  parsed: ParsedNewsletterUnsubscribeToken;
}> {
  const config = getRuntimeConfig();

  const parsed = parseToken(token, config);

  const lead = await getLead(parsed.dataMode, parsed.emailHash);
  ...
  if (lead.emailHash !== parsed.emailHash) {
    throw new NewsletterUnsubscribeError(
      'invalid-token',
      400,
 ...
 */
export function buildNewsletterUnsubscribeUrl(emailValue: string): string {
  const config = getRuntimeConfig();
  ...
  const payload = buildTokenPayload(config.dataMode, emailHash);

  const signature = signTokenPayload(payload, config.unsubscribeSecret);

  const token = `${payload}.${signature}`;

  const url = buildEmailSiteUrl('/api/newsletter/unsubscribe');
...
export async function inspectNewsletterUnsubscribeToken(
  ...
  const { lead, parsed } = await getLeadFromToken(token);

  return {
    emailHash: parsed.emailHash,

    alreadyUnsubscribed: !lead.marketingConsent,
  };
}
...
export async function unsubscribeNewsletterLeadByToken(
  ...
  const { config, lead, parsed } = await getLeadFromToken(token);

  const alreadyUnsubscribed = !lead.marketingConsent;

  const now = new Date().toISOString();

  const pendingRecord: NewsletterLeadRecord = {
    ...lead,

    marketingConsent: false,

    marketingPreferenceMethod: 'unsubscribe-link',
   ...
   * submission.
   */
  await saveLead(parsed.dataMode, pendingRecord);

  try {
    const resendContactId = await syncOptOutToResend(config, pendingRecord);

    const syncedAt = new Date().toISOString();

    await saveLead(parsed.dataMode, {
      ...pendingRecord,

...
function getJobStore(livemode: boolean) {
  return getStore(`maxipawz-paid-order-email-jobs-${getEnvironmentSuffix(livemode)}`, {
    consistency: 'strong',
  });
}
...
export async function queuePaidOrderEmailJob(
  ...
  const existing = await getPaidOrderEmailJob(sessionId, livemode);
...
export async function markPaidOrderEmailJobProcessing(
  ...
  if (!existing) {
    throw new Error('The paid-order email job does not exist.');
  }
  ...
  await store.setJSON(getJobKey(sessionId), record);

...
import { InventoryReservationError } from './inventory-reservation';
...
function validateReservationIdentifiers(
  ...
  if (!normalizedReservationId || !normalizedStripeSessionId.startsWith('cs_')) {
    throw new InventoryReservationError(
      400,
      'invalid-reservation',
...
async function lockReservation(
  client: Awaited<ReturnType<ReturnType<typeof getDatabase>['pool']['connect']>>,
  reservationId: string,
  stripeSessionId: string,
  ...
  const reservation = result.rows[0] as LockedReservationRow | undefined;
  ...
  if (reservation.stripe_session_id && reservation.stripe_session_id !== stripeSessionId) {
    throw new InventoryReservationError(
      409,
  ...
  if (!reservation.stripe_session_id) {
    ...
    reservation.stripe_session_id = stripeSessionId;
  }
...
async function getReservationItems(
  ...
  const items = result.rows as ReservationItemRow[];
...
async function releaseReservedItems(
  ...
  const items = await getReservationItems(client, reservationId);
  ...
  for (const item of items) {
    ...
    const inventory = inventoryResult.rows[0] as LockedInventoryRow | undefined;
...
async function consumeReservedItems(
  ...
  for (const item of items) {
    ...
    if (quantity < 1 || onHand < quantity || reserved < quantity) {
      throw new InventoryReservationError(
        500,
        'inventory-state-invalid',
...
export async function markInventoryReservationPaymentPending(
  ...
  const identifiers = validateReservationIdentifiers(reservationId, stripeSessionId);
...
export async function completeInventoryReservation(
  ...
  try {
    ...
    await consumeReservedItems(client, identifiers.reservationId);
...
export async function releaseInventoryReservationAfterPaymentFailure(
  ...
  try {
    ...
    await releaseReservedItems(client, identifiers.reservationId);
...
export async function expireInventoryReservation(
  ...
  try {
     ...
     */
    if (reservation.status === 'completed' || reservation.status === 'payment-pending') {
      await client.query('COMMIT');

...
import { lifestyleCategories, type LifestyleCategory } from '../data/shop';
...
export function isProductCategorySlug(
  ...
  return productCategorySlugs.includes(value as ProductCategorySlug);
}

const sandboxCatalogEnabled = import.meta.env.PUBLIC_SANDBOX_CATALOG_CHECKOUT === 'true';

function isProductVisible(product: Product): boolean {
  return sandboxCatalogEnabled || !product.isDemo;
}

export function getAllProducts(): Product[] {
  return sortProducts(products.filter(isProductVisible));
}

export function getActiveProducts(): Product[] {
  return sortProducts(
    products.filter((product) => product.status === 'active' && isProductVisible(product)),
  );
}
...
export function getCategoryBySlug(slug: ProductCategorySlug): LifestyleCategory | undefined {
...
export function formatPetType(petType: PetType): string {
...
export function formatProductDimensions(dimensions?: ProductDimensions): string | null {
  if (!dimensions) {
    return null;
  }

  const measurements = [dimensions.length, dimensions.width, dimensions.height].filter(
    (measurement): measurement is number => typeof measurement === 'number',
  );

  const measurementText =
    measurements.length > 0 ? `${measurements.join(' × ')} ${dimensions.unit}` : '';

  const weightText = dimensions.weight
    ? `${dimensions.weight.value} ${dimensions.weight.unit}`
    : '';

  return [measurementText, weightText].filter(Boolean).join(' · ') || null;
}
...
export function isProductPurchasable(product: Product): boolean {
  if (product.status !== 'active' || product.availability !== 'in-stock') {
    return false;
  }

  const hasDefaultStripePrice = Boolean(product.stripeProductId && product.stripeDefaultPriceId);

  const hasVariantStripePrice = Boolean(
    product.stripeProductId &&
      product.variants?.some(
        (variant) => variant.availability !== 'discontinued' && Boolean(variant.stripePriceId),
      ),
  );

  return hasDefaultStripePrice || hasVariantStripePrice;
}

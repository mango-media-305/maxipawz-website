...
} from '../../src/types/inventory-reservation';
...
function isIndeterminateStripeCreationError(
  error: unknown,
): boolean {
  return (
    error instanceof
...
function getInventoryErrorResponse(
  error:
    InventoryReservationError,
): {
  status: number;
  ...
  error.code
    ...
    case 'inventory-state-invalid':
      return {
        ...
        message:
          'Inventory for one or more products is temporarily unavailable.',
      };
    ...
    case 'reservation-conflict':
      return {
        ...
        message:
          'Checkout inventory could not be prepared. Please try again.',
      };
  }
}
...
/**
 * Stripe Checkout metadata for campaign attribution.
 *
 * parseCheckoutRequest already sanitized all values.
 * We still build a fixed allowlist here so arbitrary
 * browser properties can never become Stripe metadata.
 */
...
async function releaseReservationAfterFailedStripeCreation(
  ...
  try {
    await releaseInventoryReservationById(
      reservationId,
...
async function expireSessionAndReleaseReservation(
  ...
  return await releaseReservationAfterFailedStripeCreation(
    reservationId,
  );
}
...
export default async function handler(
  ...
  try {
    ...
    /*
     * This metadata has no influence on checkout validation,
     * inventory, pricing, shipping, tax, or reservations.
     */
        ...
        await stripe.checkout.sessions.create(
          {
            ...
            metadata: {
              ...
              merchandise_subtotal_cents:
                String(
                  cart.merchandiseSubtotalAmount,
                ),
          ...
          {
            idempotencyKey:
              `maxipawz-checkout-${cartReference}`,
          },
        );
    } catch (
    error
      ...
      if (
        ...
        if (
          isIndeterminateStripeCreationError(
            error,
          )
        ) {
          /*
          ...
          await releaseReservationAfterFailedStripeCreation(
            inventoryReservation.id,
          );
        }
      }

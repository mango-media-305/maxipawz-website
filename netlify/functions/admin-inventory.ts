...
import type { AdminInventoryResponse } from '../../src/types/admin-inventory';

function jsonResponse(body: AdminInventoryResponse, status = 200): Response {
...
function getSuccessMessage(
  action: 'provision' | 'adjust-on-hand' | 'set-on-hand' | 'set-thresholds',
): string {
  switch (action) {
    case 'provision':
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (request.method === 'GET') {
      const inventory = await listAdminInventoryState();
    ...
    const payload = parseAdminInventoryMutationRequest(rawRequest);

    const result = await executeAdminInventoryMutation(payload);
    ...
    return jsonResponse({
      ...
      message: getSuccessMessage(result.action),
    });
  } catch (error) {
    ...
    console.error('Admin inventory operation failed.', error);

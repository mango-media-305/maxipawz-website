...
function authorizeRequest(request: Request): boolean {
  const expectedSecret = Netlify.env.get('MAXIPAWZ_INTERNAL_FUNCTION_SECRET')?.trim();

  const providedSecret = request.headers.get('X-MaxiPawz-Internal-Secret')?.trim();

  if (!expectedSecret || expectedSecret.length < 32 || !providedSecret) {
    return false;
  }

  return safeSecretEquals(expectedSecret, providedSecret);
}
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('Manual inventory reservation reconciliation failed.', error);

    return jsonResponse(
      {
        ok: false,

        error: 'Inventory reservation reconciliation failed.',
      },
      500,
    );
  }
...
export const config: Config = {
  path: '/api/internal/reconcile-inventory-reservations',
};

    ...
    try {
        ...
        if (!order) {
            return errorResponse(404, {
                ...
                message:
                    'Payment confirmation has not reached the order system yet.',
            });
        }

        return jsonResponse(
            toPublicOrderStatus(order),
        );
    } catch (error) {
        console.error(
